...
class HostRequestsViewController: UIViewController, UITableViewDataSource, UITableViewDelegate
{
    ...
    func getGuestInfoForHost(currentDateTime: String)
    {
        ...
        ref.observeSingleEvent(of: .value, with: { (snapshot) in
            ...
            DatabaseFunctions.guestInfoForHost(currentDateTime: currentDateTime, guestUID: self.guestUID!, hostUID: self.hostUID!, requestID: self.requestID!, name: name, email: email, number: number)
        })
    }
    ...
    func getHostInfoForGuest(currentDateTime: String)
    {
        ...
        ref.observeSingleEvent(of: .value, with: { (snapshot) in
            ...
            DatabaseFunctions.hostInfoForGuest(currentDateTime: currentDateTime, guestUID: self.guestUID!, hostUID: self.hostUID!, name: name, email: email, number: number)
        })
    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let approveButton = UIBarButtonItem(title: "Approve", style: .plain, target: self, action: #selector(approveAction))
    ...
    override func viewWillAppear(_ animated: Bool) {
        if let user = Auth.auth().currentUser
        {
            ...
            ref.child(user.uid).child("requests").queryOrderedByKey().observeSingleEvent(of: .value, with: { (snapshot) in
                for snap in snapshot.children
                {
                    ...
                    self.options.append((dates: "\(checkInDate) - \(checkOutDate)", guestUID: guestUID, hostUID: user.uid, requestID: offerSnap.key))
                }
